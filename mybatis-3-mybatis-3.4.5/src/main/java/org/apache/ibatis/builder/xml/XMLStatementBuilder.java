...
public class XMLStatementBuilder extends BaseBuilder {
	...
	public void parseStatementNode() {
		//获取SQL节点的id以及databaseId属性，若其databaseId属性值与当前使用的数据库不匹配，咋不加载
		//该sql节点，若存在相同的id且databaseid不为空的sql节点，咋不在加载该sql节点
		...
		//这里会检测sql节点中是否配置了selectKey节点，SQL节点的属性值useGeneratedKeys、
		//mybatis-config.xml中的全局useGeneratedKeys配置，以及是否为insert语句，决定使用的
		//keyGenerator接口实现
	...
	private void parseSelectKeyNode(String id, XNode nodeToHandle, Class<?> parameterTypeClass,
		...
		//通过MapperBuilderAssistant创建MappedStatement对象，并添加到Configuration.mappedStatement
		//集合中保存，该集合为StrictMap<MappedStatement>类型
		builderAssistant.addMappedStatement(id, sqlSource, statementType, sqlCommandType, fetchSize, timeout,
				parameterMap, parameterTypeClass, resultMap, resultTypeClass, resultSetTypeEnum, flushCache, useCache,
		...
		//创建selectKey节点对应的KeyGenerator,并添加到Configuration.keyGenerators集合中保存
		//Configuration.keyGenerators字段StrictMap<KeyGenerator>类型
