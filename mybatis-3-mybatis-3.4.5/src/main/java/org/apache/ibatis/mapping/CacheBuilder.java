...
public class CacheBuilder {
	//Cache对象的唯一标识，一般对应映射文件中的配置namespace
	private final String id;
	//Cache接口的真正实现类，默认是前面介绍的PerpetualCache
	...
	//根据上述字段的值创建Cache对象并添加合适的装饰器
	public Cache build() {
		//设置默认值
		setDefaultImplementations();
		//根据implementation字段指定的类型，通过反射获取参数为String类型的构造方法，并通过
		//该构造方法创建Cache对象
		Cache cache = newBaseCacheInstance(implementation, id);
		//根据cache节点下的配置property信息，初始化Cache对象
		setCacheProperties(cache);
		// issue #352, do not apply decorators to custom caches
		//检测cache对象的类型，如果是PerpetualCache类型，则添加decorators集合中的装饰器
		//如果是自定义类型的Cache接口实现，则不添加decorators集合中的装饰器
		if (PerpetualCache.class.equals(cache.getClass())) {
			for (Class<? extends Cache> decorator : decorators) {
				//通过反射获取Cache类型的构造方法，并通过构造方法创建装饰器
	...
	private void setDefaultImplementations() {
		//如果implementation和decorators字段为空，则设置默认值 
		if (implementation == null) {
			//implementation字段设置默认值为PerpetualCache
			implementation = PerpetualCache.class;
			if (decorators.isEmpty()) {
				//decorators字段设置默认值为LruCache.class
	...
	//根据Cache节点下的配置的proerties信息，初始化Cache对象
